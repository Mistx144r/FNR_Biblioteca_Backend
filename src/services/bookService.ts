...
function saveBookInCache(bookId: number, data: Book) {
  return redis
    .multi()
    .set(
      `${env.NODE_ENV}:Book:${bookId}`,
      JSON.stringify(serializeBigInt(data)),
    )
    .expire(`${env.NODE_ENV}:Book:${bookId}`, 1800)
    .exec();
}
...
function savePaginatedBooksInCache(
  ...
  return redis
    .multi()
    .set(
      `${env.NODE_ENV}:Book:${pages}:${limit}`,
      JSON.stringify(serializeBigInt(data)),
    )
    .expire(`${env.NODE_ENV}:Book:${pages}:${limit}`, 900)
    .exec();
}
...
export async function getAll(
  ...
  const where: Prisma.BookWhereInput = {
    ...
    ...(possibleFilters?.published_at && {
      published_at: possibleFilters.published_at,
    }),
  };
